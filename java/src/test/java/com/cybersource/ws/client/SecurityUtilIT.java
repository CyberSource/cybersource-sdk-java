...
public class SecurityUtilIT {
    ...
    @Test
    public void testIdentityToKeyStoreOptionalCaching() throws Exception{
        merchantProperties.setProperty("keyFilename", merchantProperties.getProperty("keyAlias") + ".p12");

        // caching enabled (default)
        final MerchantConfig configCachingEnabled =
                new MerchantConfig(merchantProperties, "merchant_id_optional_caching_test");
        final MerchantConfig configCachingEnabledSpy = Mockito.spy(configCachingEnabled);

        SecurityUtil.loadMerchantP12File(configCachingEnabledSpy, logger);
        SecurityUtil.loadMerchantP12File(configCachingEnabledSpy, logger);

        verify(configCachingEnabledSpy, times(1)).getKeyFile();

        // caching disabled
        merchantProperties.setProperty("identitiesCacheEnabled", "false");
        final MerchantConfig configCachingDisabled =
                new MerchantConfig(merchantProperties, "merchant_id_optional_caching_test");
        final MerchantConfig configCachingDisabledSpy = Mockito.spy(configCachingDisabled);

        SecurityUtil.loadMerchantP12File(configCachingDisabledSpy, logger);
        SecurityUtil.loadMerchantP12File(configCachingDisabledSpy, logger);

        verify(configCachingDisabledSpy, times(2)).getKeyFile();
    }
